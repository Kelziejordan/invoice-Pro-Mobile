...
import { calculateSubtotal, calculateTax, calculateTotal, getItemAmount } from '../../utils/calculations';
                ...
                <tbody className="text-slate-700">
                  {invoice.items.map((item) => (
                    <tr key={item.id} className="border-b border-slate-100">
                      ...
                      <td className="py-4 px-2 text-right font-medium text-slate-900 align-top">{formatCurrency(getItemAmount(item), currency)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
